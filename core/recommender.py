...
def _score(activity, inquiry, phase, used_titles):
    score = activity.base_score
    if activity.style == inquiry.travel_style: score += 4
    if activity.interest in inquiry.interests: score += 5
    if activity.day_suitability in (phase, 'Any'): score += 2
    if activity.time_slot in ('Flexible', 'Full-day'): score += 1
    if activity.name in used_titles: score -= 3
    return score
    ...
    for day in range(1, inquiry.duration_days + 1):
        ...
        for slot in SLOTS:
            ranked = sorted(
                activities,
                key=lambda a: (
                    -(_score(a, inquiry, phase, used) + (2 if a.time_slot in (slot, 'Flexible', 'Full-day') else 0)),
                    a.intensity,
                    a.name,
                )
            )
            ...
            ItineraryItem.objects.create(
                ...
                time_slot=slot,
                title=pick.name,
                notes=f"{phase} day • {pick.intensity} intensity • {pick.style}",
            )

    itinerary.summary = f"Draft {inquiry.duration_days}-day itinerary for {inquiry.destination.name} ({inquiry.travel_style})."
    itinerary.save()
    return itinerary
